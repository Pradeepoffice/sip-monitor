...
const C = {
  ...
  bg: "#080f1a", card: "#0f1c2e", border: "#1e293b",
...
function Badge({ status }) {
  const map = {
    UP: [C.green, "#0d3b2e"], DOWN: [C.red, "#3b0d0d"],
    WARNING: [C.yellow, "#3b2e0d"], GOOD: [C.green, "#0d3b2e"],
    CRITICAL: [C.red, "#3b0d0d"], UNKNOWN: [C.muted, C.dim],
  };
...
function SectionTitle({ children }) {
  return <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 14 }}>{children}</div>;
}
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.border}`, padding: "14px 28px", background: "#0b1422", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={() => { setLoading(true); fetchAll(); }} style={{ background: C.dim, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 6, padding: "5px 12px", fontSize: 11, cursor: "pointer" }}>
            {loading ? "…" : "↻ Refresh"}
          </button>
        </div>
      </div>
          ...
          {alerts.length === 0
            ...
            : <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 200, overflowY: "auto" }}>
              {alerts.slice(0, 10).map((a, i) => (
                <div key={i} style={{ background: C.card, border: `1px solid ${a.severity === "CRITICAL" ? C.red + "44" : C.yellow + "44"}`, borderLeft: `3px solid ${a.severity === "CRITICAL" ? C.red : C.yellow}`, borderRadius: 8, padding: "8px 14px", display: "flex", gap: 12, alignItems: "center" }}>
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 14 }}>
        <KPICard label="Total Calls Today" value={num(ov.totalToday)}    color={C.blue}   icon="📞" sub="All calls today" />
        <KPICard label="Agent Calls"       value={num(ov.agentCalls)}    color={C.orange} icon="🎧" sub="Transferred to Agent" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              ...
              { label: "Agent Calls", value: ov.agentCalls, color: C.orange },
        ...
        <Card>
          ...
          <div style={{ fontSize: 13, color: C.muted, marginBottom: 8 }}>Last CDR fetch from Exotel</div>
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 20 }}>
        {clients.map((c) => (
          <Card key={c.client}>
            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>🏢 {c.client}</div>
            <div style={{ fontSize: 11, color: C.muted, fontFamily: "monospace", marginBottom: 12 }}>{c.did}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <div style={{ background: C.dim, borderRadius: 8, padding: "10px" }}>
                <div style={{ fontSize: 10, color: C.muted }}>Total Calls</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: C.blue }}>{num(c.total)}</div>
              </div>
              <div style={{ background: C.dim, borderRadius: 8, padding: "10px" }}>
                <div style={{ fontSize: 10, color: C.muted }}>Agent Calls</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: C.orange }}>{num(c.agentCalls)}</div>
              </div>
              <div style={{ background: C.dim, borderRadius: 8, padding: "10px" }}>
              ...
              <div style={{ background: C.dim, borderRadius: 8, padding: "10px" }}>
                ...
                <div style={{ fontSize: 16, fontWeight: 700, color: connColor(c.connectivity) }}>{pct(c.connectivity)}</div>
              </div>
            </div>
            <div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", fontSize: 11, color: C.muted }}>
              <span>📥 {num(c.inbound)}</span>
              <span>📤 {num(c.outbound)}</span>
      ...
      <Card>
        <SectionTitle>Client Comparison — Total vs Agent Calls</SectionTitle>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={clients}>
            ...
            <Bar dataKey="total"      fill={C.blue}   name="Total Calls" radius={[4,4,0,0]} />
            <Bar dataKey="agentCalls" fill={C.orange} name="Agent Calls" radius={[4,4,0,0]} />
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                {["DID", "Client", "Total Calls", "Agent Calls", "Connected", "Failed", "Connectivity", "Avg Duration", "Health"].map((h) => (
            ...
            <tbody>
              {dids.map((d) => (
                <tr key={d.did} style={{ borderBottom: `1px solid ${C.border}22` }}>
                  ...
                  <td style={{ padding: "10px 12px", color: C.orange, fontWeight: 600 }}>{num(d.agentCalls)}</td>
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 14 }}>
        <SectionTitle>Hourly Volume — Total vs Agent Calls</SectionTitle>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={hourly}>
            <XAxis dataKey="label" tick={{ fill: C.muted, fontSize: 10 }} interval={1} />
            <YAxis tick={{ fill: C.muted, fontSize: 11 }} />
            <Tooltip contentStyle={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }} />
            <Bar dataKey="connected"  fill={C.green}  name="Connected"   stackId="a" />
            <Bar dataKey="agentCalls" fill={C.orange} name="Agent Calls" stackId="a" />
...
function KPITab({ kpi }) {
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 14 }}>
        <SectionTitle>Today — Hourly Connectivity vs 98% Target</SectionTitle>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={(kpi.hourly || []).filter((h) => h.total > 0).map((h) => ({ ...h, target: 98 }))}>
            ...
            <Tooltip contentStyle={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }} />
            <Line type="monotone" dataKey="connectivity" stroke={C.blue} strokeWidth={2} dot={false} name="Connectivity %" />
            <Line type="monotone" dataKey="target" stroke={C.red} strokeWidth={1.5} strokeDasharray="4 4" dot={false} name="Target 98%" />
          </LineChart>
        </ResponsiveContainer>
      </Card>
      <Card>
