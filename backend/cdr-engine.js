...
async function fetchCDR() {
  ...
  if (!accountSid || !apiKey || !apiToken) { console.log("[CDR] Credentials not set."); return; }
  try {
    const now = new Date();
    ...
    let fetchedCalls = [], afterCursor = null, page = 1, totalCount = 0;
    ...
    while (page <= 50) {
      ...
      const data = res.data;
      if (!data?.Calls?.length) { console.log(`[CDR] Page ${page}: No more calls.`); break; }
      if (page === 1) { totalCount = data?.Metadata?.Total || 0; console.log(`[CDR] Total: ${totalCount}`); }
      fetchedCalls = [...fetchedCalls, ...data.Calls];
      console.log(`[CDR] Page ${page}: ${data.Calls.length} (total: ${fetchedCalls.length})`);
      const nextUri = data?.Metadata?.NextPageUri;
      if (!nextUri) { console.log(`[CDR] Done. ${fetchedCalls.length} fetched.`); break; }
    ...
    fetchError = null;
    console.log(`[CDR] Done! ${totalFetched}/${totalCount} calls.`);
  } catch (err) {
    fetchError = err.response ? `HTTP ${err.response.status}: ${JSON.stringify(err.response.data)}` : err.message;
...
function cleanNumber(num) { if (!num) return ""; return num.toString().replace(/^sip:/i, "").replace(/[^0-9+]/g, ""); }
function isSuccess(s) { return s === "completed"; }
function isFailure(s) { return ["failed", "busy", "no-answer", "canceled"].includes(s); }
function callsToday() { const s = new Date(); s.setHours(0,0,0,0); return allCalls.filter((c) => c.dateCreated && c.dateCreated >= s); }
function callsInWindow(m) { const c = new Date(Date.now() - m*60*1000); return allCalls.filter((c) => c.dateCreated && c.dateCreated >= c); }
function connectivity(calls) { if (!calls.length) return 0; return parseFloat(((calls.filter((c) => isSuccess(c.status)).length / calls.length) * 100).toFixed(2)); }
function avgDuration(calls) { const d = calls.filter((c) => isSuccess(c.status) && c.duration > 0); if (!d.length) return 0; return Math.round(d.reduce((a,b) => a+b.duration, 0) / d.length); }
function statusBreakdown(calls) { const m = {}; calls.forEach((c) => { m[c.status] = (m[c.status]||0)+1; }); return m; }
...
function getOverview() {
  ...
  return {
    totalToday: today.length, totalFetched,
...
function getClientStats() {
  ...
  return Object.entries(clientMap).map(([name, config]) => {
    ...
    return {
      client: name, did, trunkId: trunkId || "",
      total: calls.length,
      connected: bd["completed"] || 0,
...
function getDIDHealth() {
  ...
  const didMap = {};
  const didTrunkMap = {};
  Object.values(clientMap).forEach(({ did, trunkId }) => { if (did && trunkId) didTrunkMap[did] = trunkId; });
  today.forEach((c) => { if (!c.did) return; if (!didMap[c.did]) didMap[c.did] = []; didMap[c.did].push(c); });
...
function getHourlyTraffic() {
  const today = callsToday();
  const hours = {};
  for (let h = 0; h < 24; h++) hours[h] = { hour: h, label: `${pad(h)}:00`, total: 0, connected: 0, failed: 0, inbound: 0, outbound: 0, agentCalls: 0 };
  today.forEach((c) => {
  ...
  return Object.values(hours).map((h) => ({ ...h, connectivity: h.total ? parseFloat(((h.connected/h.total)*100).toFixed(1)) : 0 }));
}
...
function getConnectivityKPI() {
  ...
  const dailyMap = {};
  last7d.forEach((c) => { if (!c.dateCreated) return; const day = c.dateCreated.toISOString().split("T")[0]; if (!dailyMap[day]) dailyMap[day] = []; dailyMap[day].push(c); });
  return {
    today: connectivity(today), last7d: connectivity(last7d), last30d: connectivity(last30d),
    target: 98, todayTotal: today.length, last7dTotal: last7d.length,
    hourly: getHourlyTraffic(),
    daily: Object.entries(dailyMap).map(([day, calls]) => ({ day, connectivity: connectivity(calls), total: calls.length })).sort((a,b) => a.day.localeCompare(b.day)),
  };
}
...
function checkAlertConditions() {
  ...
  if (today.length > 10 && conn < 98) alerts.push({ type: "CONNECTIVITY", severity: conn < 95 ? "CRITICAL" : "WARNING", message: `Overall connectivity dropped to ${conn}% (target: 98%)`, value: conn, time: new Date().toISOString() });
  const failedLast15 = last15.filter((c) => isFailure(c.status)).length;
  if (failedLast15 > 100) alerts.push({ type: "FAILURE_SPIKE", severity: "CRITICAL", message: `${failedLast15} failed calls in last 15 minutes!`, value: failedLast15, time: new Date().toISOString() });
  if (hour >= 9 && hour <= 21 && last30.length === 0) alerts.push({ type: "SILENCE", severity: "WARNING", message: "No calls in last 30 minutes during business hours!", value: 0, time: new Date().toISOString() });
...
function getClientForDID(did) {
  ...
  for (const [client, config] of Object.entries(map)) { if (config.did === did) return client; }
  return "Unknown";
}

function getAllStats() {
  return {
    overview: getOverview(), clients: getClientStats(), didHealth: getDIDHealth(),
    hourly: getHourlyTraffic(), failures: getFailureAnalysis(), kpi: getConnectivityKPI(),
    alerts: checkAlertConditions(), lastFetched, fetchError,
  };
}
